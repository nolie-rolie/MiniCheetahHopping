...
% angled platforms
angled_platform1_xLB = -0.2;
angled_platform1_xUB = 1.2;
angled_platform1_yLB = platform1_yUB + 0.2;
angled_platform1_yUB = angled_platform1_yLB + 0.5;
...
for j = 1:nHops
    constr = [constr, ...
              implies(zTD(3,j),qCOM(:,1,j) == qTD(1:3,j) + COM),...
              implies(zTD(3,j),qTD(4,j) == 0), ...
              implies(zTD(3,j),qTD(5,j) == 0)];
end

% angled platform
XZ = implies(1-zTD(3,:),qTD(3,1:nHops) == angled_platform1_z - n1(2)/n1(3)*(qTD(2,1:nHops) - (angled_platform1_yUB-angled_platform1_yLB)));
constr = [constr,...
          qTD(1,1:nHops) >= angled_platform1_xLB + radius_platform + safety - bigM*zTD(3,:), ... 
          qTD(1,1:nHops) <= angled_platform1_xUB - radius_platform - safety + bigM*zTD(3,:), ... 
          qTD(2,1:nHops) >= angled_platform1_yLB + radius_platform + safety - bigM*zTD(3,:), ... 
          qTD(2,1:nHops) <= angled_platform1_yUB - radius_platform - safety + bigM*zTD(3,:), ...
          XZ];
% configuration constraint for angled platforms
for j = 1:nHops
    constr = [constr, ...
              implies(1-zTD(3,j),qCOM(:,1,j) == qTD(1:3,j) + n1),...
              implies(1-zTD(3,j),qTD(4,j) == angled_platform1_roll), ...
              implies(1-zTD(3,j),qTD(5,j) == 0)];
end
...
objective = 0;
for i = 1:nHops+1
    objective = objective + A*fnet(3,i);
    objective = objective + B*fnet(1,i)^2 + D*fnet(2,i)^2;
end
...
force_scale = 0.001;
f = GetContactForcesFWP(value(fnet), value(n));
