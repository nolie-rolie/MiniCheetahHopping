...
for indStance = 2:2:nStance*2-3
    for i = floor(wSt*N(indStance)):floor(wEnd*N(indStance))
        constr = [constr, ...
                  ...
                  x(1:3,stance_start(indStance)+i) == xd(1:3,stance_start(indStance)+i), ...
                  x(4:6,stance_start(indStance)+i) == mean(reshape(pfd(:,stance_start(indStance)+i),3,4),2) + n1, ...
...
save("AngledPlatformTrajectoryDataWithCOM.mat",'trajGRF','trajCOM','pfd');

%% save COM, footstep, and GRF trajectories to compare to sim
save("AngledPlatformTrajectoryWithCOMToCompare.mat",'x','v','pfd','u','F');
