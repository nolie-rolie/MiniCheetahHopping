...
maxHeightDiff = robot.hipLinkLength + robot.kneeLinkLength - 0.18;

%% problem setup

% import trajectory

load('ObstacleTrajectoryForNLP.mat')
...
for i = 1:control_traj_len
    if isequal(ctacts(:,i),ones(4,1))
        constr = [constr, ...
                  -0.01 <= x(1,i) - xd(1,i) <= 0.01, ...
                  -0.01 <= x(2,i) - xd(2,i) <= 0.01, ...
                  -0.01 <= x(3,i) - xd(3,i) <= 0.01, ...
                  x(6,i) <= xd(6,i) + 1/2*maxHeightDiff,...
                  x(6,i) >= xd(6,i) - 1/2*maxHeightDiff];
    end
end
...
for i = 1:control_traj_len
    ...
    for j = 1:nCtacts
        constr = [constr, ...
                  nc(3*(j-1)+1:3*j,i) == skew(pfd(3*(j-1)+1:3*j,i) - x(4:6,i))*(u(3*(j-1)+1:3*j,i))];
    end
...
%                          x(7:9,i+1) == x(7:9,i) + h * ((Rz*Ry*Rx)*H(1:3,1:3)*(Rz*Ry*Rx)'*(F(1:3,i)-cross(x(7:9,i),(Rz*Ry*Rx)*I*(Rz*Ry*Rx)'*x(7:9,i)))), ...
                         x(7:9,i+1) == x(7:9,i) + h * (Rz*H(1:3,1:3)*Rz'*F(1:3,i)), ...
                         x(10:12,i+1) == x(10:12,i) + h * (H(4:6,4:6)*F(4:6,i) - g(4:6))
              ];
...
% stance = 1;
% stance_start = ones(nStance*2-2,1);
...
%             stance = 0;
%             
%             if (indStance ~= 1 && indStance ~= size(alpha,3) && mod(indStance,2) == 0)
%                 for phase = 1:2
%                     if phase == 1
%                         start = 1;
%                         stop = floor(wSt*N(indStance));
%                     else
%                         start = floor(wEnd*N(indStance));
%                         stop = N(indStance);
%                         indStance = indStance + 1;
%                         stance_start(indStance) = stance_start(indStance-1);
%                     end
%                     for j = start:stop
%                         for k = 1:n+1
%                             B{indStance}(k,j-start+1) = nchoosek(n,(k-1))*(1-(j-start)/(stop-start))^(n-(k-1))*((j-start)/(stop-start))^(k-1);
%                         end
% 
%                         constr = [constr, ...
%                                   F(:,(j-1)+stance_start(indStance)) == alpha(:,:,indStance)*B{indStance}(:,j-start+1)
%                                   ];
%                     end
%                 end
%             else
...
% wait
% for indStance = 2:2:nStance*2-3
%     for i = floor(wSt*N(indStance)):floor(wEnd*N(indStance))
%         constr = [constr, ...
%                   F(1:5,stance_start(indStance)+i) == zeros(5,1), ...
%                   u(1:2,stance_start(indStance)+i) == zeros(2,1),u(4:5,stance_start(indStance)+i) == zeros(2,1), ...
%                   u(7:8,stance_start(indStance)+i) == zeros(2,1),u(10:11,stance_start(indStance)+i) == zeros(2,1), ...
%                   x(3,stance_start(indStance)+i) == xd(3,stance_start(indStance)+i), ...
%                   x(4:6,stance_start(indStance)+i) == mean(reshape(pfd(:,stance_start(indStance)+i),3,4),2) + COM, ...
%                   x(7:12,stance_start(indStance)+i) == zeros(6,1)];
%     end
% end
...
graphic_option.show_angled_platforms = false;
...
graphics_data.platforms = platforms;
graphics_data.obstacles = obstacles;
graphics_data.CameraPos = [0,-5,2];
graphics_data.CameraPos = [-5,-6,2];

% call visualize function
out = visualizeMCTrajectory(graphics_data, graphic_option);

%% Save trajectory to text file for Cheetah Simulator
% v = value(x(10:12,1:end-1));
% x = out(1:6,:);
% q = out(7:end,:);
% u = value(u);
% F = value(F);
% alpha = value(alpha);
% 
% trajGRF = saveTrajToFile(q,u);
% trajCOM = saveCOMTrajToFile(x(1:3,:),v);
% 
% save("ObstacleTrajectoryDataWithCOM.mat",'trajGRF','trajCOM','pfd');
% 
% %% save COM, footstep, and GRF trajectories to compare to sim
% save("ObstacleTrajectoryWithCOMToCompare.mat",'x','v','pfd','u','F');
